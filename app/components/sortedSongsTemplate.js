...
const { sortedSongsTemplate } = require(`../assets/components.js`)
...
class AQUASortedSongsTemplate extends AQUA {
  constructor(options = {
    inStates: {
      sortedData: { en: ``, zh: `` },
      sortBy: ``,
    ...
    const listEl = (key, song) => {
      if (this.inStore.states.sortType === song.genre) {
        const singleSongList = new AQUASingleSong({
          inStates: {
            path: song.path,
            name: song.title,
            artist: song.artist,
    ...
    const renderGroups = (sorted) => {
      if (Array.isArray(sorted)) {
        sorted.forEach(group => {
          ...
          if (groupEL.outStore.states.length !== 0)
    ...
    this.addEventListener(`play`, e => {
      ...
      const currentListPaths = groupEls.map(groupEL => groupEL.outStore.states.paths).flat()
    ...
    //主题色同步
    const groupEls = Array.from(this.root.querySelectorAll(`aqua-group`))
    groupEls.forEach(groupEl => groupEl.states.groupNameColor = this.inStore.states.themeColor)
    this.inStore.watch(`themeColor`, themeColor => {
      const groupEls = Array.from(this.root.querySelectorAll(`aqua-group`))
      groupEls.forEach(groupEl => groupEl.states.groupNameColor = themeColor)
    })
  }
}

customElements.define(`aqua-sorted-songs-template`, AQUASortedSongsTemplate)

module.exports = AQUASortedSongsTemplate