const { add } = require(`../assets/components.js.js`)
const { modifyPlayLists } = require(`../loadSongs.js.js`)
const { storeStates, shared, listNames } = require(`../states.js.js`)
const ebus = require(`../utils/eBus.js.js`)
const states = storeStates.states
const icons = require(`../assets/icons.js.js`)
...
class AQUAAdd extends HTMLElement {
    ...
    menu.addEventListener(`click`, async (e) => {
      ...
      if (isItemBtn) {
        ...
        if (e.target.dataset.key) {
          ...
          if (index >= 0) {
            await modifyPlayLists(
              `addToList`,
              shared.songsToAdd,
              listNames.list[index][0]
            )
  ...
  connectedCallback() {
    modifyPlayLists(`getNames`).then((names) => {
      listNames.changeSource(names)
    })
  }
}
