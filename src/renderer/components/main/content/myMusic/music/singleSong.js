...
class SingleSong extends BaseCustomElement {
  constructor() {
    super()
    ...
    this.props.watch('hoverColor', (color) => {
      this.rootEl.style.setProperty('--hoverColor', color)
    })
    this.props.watch('hoverBgColor', (color) => {
      this.rootEl.style.setProperty('--hoverBgColor', color)
    })
    this.props.watch('nameColor', (color) => {
      this.rootEl.style.setProperty('--nameColor', color)
    })
    ...
    this.props.watch('isPlaying', (isAdd) => {
      if (isPlaying) {
        ...
        this.rootEl.style.setProperty(
          '--nameColor',
          this.inStore.states.nameColor
        )
        this.rootEl.style.setProperty(
          '--attributesColor',
          this.inStore.states.attributesColor
        )
        this.rootEl.style.setProperty(
          '--hoverColor',
          this.inStore.states.hoverColor
        )
      }
    })
    //-------------//
...
export default createEl(SingleSong)
